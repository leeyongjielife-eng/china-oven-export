...
- 日量、禁名单、9/13 线 A、不进 `today.md` P0 → **只读 cofounder 该节**，本文件不重复数字。
...
| **A 软切（2026-08-02→9/13）** | **暂停日扩 +5**（库存闸门例外仍有效）；默认跟进 **仅 A**；不改 `phase_id` |
...
| Week 3 **A 软切 + 选项 2**（至 **2026-09-13**） | **0**（仅当 `A Lead < 20` 才强制补 A）；**禁止因 LI 新增公司行** | **15**/工作日：**优先 A Lead 首封**；可穿插 A Contacted 跟进；禁 B/C | 见 cofounder LinkedIn 触达政策（只打已有 A 行） |
