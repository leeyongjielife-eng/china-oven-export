...
| 阶段 | Phase 0 验证期，0 客户；买家 **92** · 已联系 **84/100** |
...
- **瓶颈:** **可发邮箱 Lead 池已空**（2026-07-21 发完 #80–#92）；已联系 **85/100**；距 W4 还差 Contacted≥100 **且** 回复≥5（当前回复 **1**）
- **优势:** 买家 **92**；Gmail MCP ✅；发信政策已统一（`今天发` · 15/工作日 · 云端不实发）；Research PR #5 已合入
- **今日主线:** 云端/本机 Sales **先扩有邮箱买家 #93+**（至少 +5，目标凑满明日 15）；再对齐草稿
...
## 今日排期（2026-07-24 · Founder 排程 · W3）

**主题：补跑跟进 #19–#33（昨日未发）+ Sales 续扩 #100+**

| 优先级 | 谁 | 任务 | 状态 |
|:---:|------|------|:---:|
| **P0** | **Co-founder / 本机** | **直接发** 跟进 **#19–#33** 共 15（短文、主题无 `$`） | ✅ **15/15** |
| **P0** | **Sales** | 换画像扩买家从 **#100** 起 ≥5 + 草稿 | ⏳ `today.md` pending |
| ⏸️ | — | 不问叔叔；不切 Research Reset；#6/#17 Bounce 不自动补 | ✅ |

**Founder 阶段检查：** 保持 `W3_OUTREACH`；KPI=真回复；已联系 **84** · 回复 **1**。

### Co-founder 今日说明

- 已授权：**发信不用再说「今天发」，本次直接发**
- 队列：`#19–#33`（补 7/23 未跑的跟进批）
- 新冷邮 `#93–#98` 留到跟进批之后

### 明日排期（2026-07-25 · 周六 · W3）

| 优先级 | 指派 | 任务 | 目标 |
|:---:|------|------|------|
| **P0** | **本机** | 跟进 `#34–#48`（跳过无邮箱）和/或冷邮 `#93–#98` | 推回复 / Contacted↑ |
| **P1** | **Sales** | 继续换画像扩买家 | 可发池 |

---

## 上日排期（2026-07-23 · W3）✅ Sales 完成 · 本机跟进未跑

| 优先级 | 谁 | 任务 | 状态 |
|:---:|------|------|:---:|
| **P0** | **Co-founder（本机）** | 跟进 #19–#33 | ❌ 未触发 → **今日补跑** |
| **P0** | **Sales** | #93+ 换画像 | ✅ #93–#99 · `today.md` done |
...
| P1 | 外联 100（定价话术见 `product.md` 定价策略） | Co-founder 本机跟进/冷邮 + Sales 草稿 | W3；KPI=真回复；已联系 **84** · 回复 **1** · 今日跟进 #19–#33 |
